...
constructor(
    ...
    private fun loadCartItems() {
        viewModelScope.launch {
            try {
                ...
                _state.update { it.copy(items = items, totalPrice = total, error = null) }
            } catch (e: Exception) {
    ...
    fun onDateSelected(dateMillis: Long) {
        _state.update { it.copy(selectedDate = dateMillis, showDatePicker = false, error = null) }
    }

    fun onTimeSelected(hour: Int, minute: Int) {
        val timeString = String.format("%02d:%02d", hour, minute)
        _state.update { it.copy(selectedTime = timeString, showTimePicker = false, error = null) }
    }
    ...
    fun placeOrder() {
        if (!_state.value.canProceed) {
            _state.update { it.copy(error = "Please select date and time for pickup") }
            return
        }
